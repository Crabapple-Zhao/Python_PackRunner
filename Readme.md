...
- **自动清理**：打包成功后自动删除 build 中间目录与 `.spec` 文件，只保留 `dist` 与最终 EXE
...
从本仓库的 [Releases](../../releases) 页面下载最新版 `Python_Tool_V1.0.exe`，双击即可运行（仍需提前安装 uv）。
...
4. 点击 **"▶ 运行脚本 (Run)"** 测试脚本；
5. 验证无误后点击 **"📦 打包为 EXE"**，生成的 EXE 将保存在目标脚本同目录下的 `dist` 文件夹中。
...
- **打包参数**：默认使用 `pyinstaller --onefile`，且默认勾选 `--noconsole`。若脚本是命令行工具（需要终端窗口）或需附带资源文件，请修改源码中 `pack_exe` 函数的打包参数；
